...
class Reservation extends ASoapClient
{
    ...
    public function __construct($wsdl, $path = NULL)
    {
        ...
        if (is_array($wsdl['variable_name'])) {
            list($wsdl['variable_name'], $wsdl['type_name']) = $wsdl;
        }

        if ($wsdl['type_name'] == NULL) {
            $wsdlPath = $confTravelport['AIR_WSDL_AND_SCHEMA_PATH'] . $wsdl['variable_name'] . '.wsdl';
        } else {
            $wsdlPath = $confTravelport[$wsdl['type_name']] . $wsdl['variable_name'] . '.wsdl';
        }
