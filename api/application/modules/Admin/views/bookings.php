                        ...
                        <?php if (!empty($all_booking)) {
                            ...
                            foreach ($all_booking as $booking) : ?>
                                <tr class="xcrud-row xcrud-row-0">
                                    ...
                                    <td class="text-center">
                                        <select name="" id="payment_status" class="form-select status <?= $booking->booking_payment_status ?>" onchange="update_paybooking_status(this);" <?php if ($isSuperAdmin) {
                                                                                                                                                                                            } else {
                                                                                                                                                                                                echo "disabled";
                                                                                                                                                                                            } ?>>
                                            >
                                    ...
                                    <td><a class="btn btn-outline-dark" target="_blank" href="<?= base_url('../'); ?><?= $booking->module ?>/booking/ticket/<?= $booking->booking_ref_no ?>/<?= $booking->booking_id ?>" style="cursor: pointer;"><i class="fa fa-file"></i> &nbsp; Ticket</a></td>
                                    <?php if ($isSuperAdmin) { ?>
    ...
    function update_paybooking_status(val) {
        ...
        $.ajax({
            ...
            data: {
                booking_id: b_id,
                module: b_module,
                update_paybooking_status: b_paystatus
            },
            dataType: 'JSON',
            success: function(data) {
                if (data) {
