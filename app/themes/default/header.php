...
// APP DATA
$app = $_SESSION['app'];
foreach ($app->currencies as $curreny) {
    if ($curreny->default == true && $curreny->status == true) {
        $default_currency = $curreny->name;
    }
}
...
// MODULES CONDITIONS
foreach ($app->modules as $model) {
    if ($model->status == true && $model->name  == 'flights') {
        $flights = 1;
        $flights_order = $model->order;
    }
}
foreach ($app->modules as $model) {
    if ($model->status == true && $model->name  == 'hotels') {
        $hotels = 1;
        $hotels_order = $model->order;
    }
}
foreach ($app->modules as $model) {
    if ($model->status == true && $model->name  == 'tours') {
        $tours = 1;
        $tours_order = $model->order;
    }
}
foreach ($app->modules as $model) {
    if ($model->status == true && $model->name  == 'cars') {
        $cars = 1;
        $cars_order = $model->order;
    }
}
foreach ($app->modules as $model) {
    if ($model->status == true && $model->name  == 'visa') {
        $visa = 1;
        $visa_order = $model->order;
    }
}
foreach ($app->extras  as $model) {
    if ($model->status == true && $model->title == 'newsletter') {
        $newsletter = 1;
    }
}
foreach ($app->extras  as $model) {
    if ($model->status == true && $model->title == 'blog') {
        $blog = 1;
    }
}
foreach ($app->extras  as $model) {
    if ($model->status == true && $model->title == 'offers') {
        $offers = 1;
    }
}
...
foreach ($fils_data as $key) {
    if ($key->lang_code == $_SESSION['session_lang']) {
    ...
    <header class="header-area">
        ...
        <div class="container">
            <div class="row align-items-center">
                ...
                <?php $indir = array_filter(scandir('app/lang', 1), function ($item) {
                    return !is_dir('app/lang/' . $item);
                });
                $files_data = array();
                foreach ($indir as $key => $value) {
                            ...
                            <div class="main-menu-content w-100">
                                <div class="align-items-center d-flex justify-content-between gap-3">
                                    ...
                                    <script>
                                        ...
                                        if (segment_2 == "hotels") {
                                            $(".active_hotels").css("font-weight", "bold");
                                        }
                                        if (segment_2 == "flights") {
                                            $(".active_flights").css("font-weight", "bold");
                                        }
                                        if (segment_2 == "tours") {
                                            $(".active_tours").css("font-weight", "bold");
                                        }
                                        if (segment_2 == "cars") {
                                            $(".active_cars").css("font-weight", "bold");
                                        }
                                        if (segment_2 == "visa") {
                                            $(".active_visa").css("font-weight", "bold");
                                        }
                                        if (segment_2 == "blog") {
                                            $(".active_blog").css("font-weight", "bold");
                                        }
                                        if (segment_2 == "offers") {
                                            $(".active_offers").css("font-weight", "bold");
                                        }
                                    </script>
                                            ...
                                            <div class="header-right-action pt-1 multi_language">
                                                <div class="dropdown">
                                                    <button class="btn btn-default dropdown-toggle" type="button" id="languages" data-bs-toggle="dropdown" aria-expanded="false">
                                                        <span class="flag-icon flag-icon-<?php foreach ($files_data as $lang) {
                                                                                                if ($lang->lang_code == $session_lang) {
                                                                                                    echo $lang->country_code;
                                                                                                }
                                                                                            } ?> mr-1"></span>
                                                        <?php foreach ($files_data as $lang) {
                                                            if ($lang->lang_code == $session_lang) {
                                            ...
                                            <div class="header-right-action  pt-1 pe-2 multi_currency">
                                                <div class="dropdown">
                                                    ...
                                                    <ul class="dropdown-menu" aria-labelledby="currency" style="max-height: 260px; overflow: auto;">
                                                        <?php foreach ($app->currencies as $currencies) {
